...
class Solution {
   public:
    int cherryPickup(vector<vector<int>>& grid) {
        ...
        dp[0][0] = grid[0][0];
        for (int s = 1; s < m + n - 1; ++s) {
            vector<vector<int>> new_dp(m, vector<int>(m, -1));
            for (int r1 = max(0, s - n + 1); r1 < m && r1 <= s; ++r1) {
                for (int r2 = r1; r2 < m && r2 <= s; ++r2) {
                    int c1 = s - r1;
                    int c2 = s - r2;
                    ...
                    int v = -1;
                    if (r1 > 0 && r2 > 0) v = max(v, dp[r1 - 1][r2 - 1]);
                    if (r1 > 0 && s - r2 > 0) v = max(v, dp[r1 - 1][r2]);
                    if (s - r1 > 0 && r2 > 0) v = max(v, dp[r1][r2 - 1]);
                    if (s - r1 > 0 && s - r2 > 0) v = max(v, dp[r1][r2]);
                    if (v == -1) continue;
