...
void PrintUsageAndExit() {
    const char* usage_string =
        ...
        "  --test-data-dir       [TEST_DATA_DIR] path to the directory "
    ...
    static TestConfig FromCommandLine(
        ...
        for (size_t i = 0; i < commandline_args.size(); i++) {
            if (commandline_args[i] == "--test-data-dir") {
                config.test_data_dir = test_config::GetParam(
                    commandline_args, ++i, "--test-data-dir");
        ...
        if (!config.test_data_dir.empty()) {
            if (!platform::IsDir(config.test_data_dir.c_str())) {
                throw std::runtime_error(FmtStr(
                    "CL: --test_data_dir argument ({}) is not a directory",
